...
public:
	...
	void Init()
	{
		...
		Load("CountDown", "Sound/Countdown06-2.wav", SoundType::SE);
		Load("Click", "Sound/click.wav", SoundType::SE);
		Load("Result", "Sound/result.wav", SoundType::SE);

		//============== BGM ================
		Load("Title", "Sound/title.wav", SoundType::BGM);
		Load("BossBefore", "Sound/BossBefore.wav", SoundType::BGM);
		Load("BossAfter", "Sound/BossAfter.wav", SoundType::BGM);
	}
	...
	{
		GroupVolume(SoundType::SE, m_seVolume);
		GroupVolume(SoundType::BGM, m_bgmVolume);
	}
	...
	void GroupVolume(const SoundType& a_soundType, float a_volume)
	{
		...
		// 全インスタンスをループして該当タイプに反映
		for (auto& pair : m_instMap)
		{
			if (m_typeMap[pair.first] == a_soundType)
			{
				pair.second->SetVolume(a_volume);
			}
		}
	}
